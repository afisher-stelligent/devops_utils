'''stack_deploy.py: an application for intelligent
    stack deployment through AWS Cloudformation. See README for full details'''
import argparse
import json
import boto3
...
BAD_STATUSES = ["ROLLBACK_COMPLETE", "UPDATE_ROLLBACK_FAILED"]
...
def _stack_exists(stack_name):
    ...
    for stack in stacks:
        if stack.get('StackStatus') == 'DELETE_COMPLETE':
            continue
        if stack.get('StackStatus') in BAD_STATUSES:
...
def _process_parameters(parameter_file):
    try:
        with open(parameter_file, encoding='utf-8') as the_file:
...
def _wait_for_stack(stack_name, status):
    stack_waiter = cf.get_waiter(stack_name)
    stack_waiter.wait(stack_name, status)


if __name__ == '__main__':
    arg_parser = argparse.ArgumentParser(
        description='Create or update a stack using AWS CloudFormation'
    )

    arg_parser.add_argument(
        '-n', '--stack_name', help='Name of the stack to create or update', required=True
    )
    arg_parser.add_argument('-t', '--template_file',
        help='Path to file in YAML format', required=True
    )
    arg_parser.add_argument('-p', '--parameter_file',
        help='Path to json parameter file for template parameters.'
    )
